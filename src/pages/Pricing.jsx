...
export default function Pricing() {
  ...
  const pricingData = {
    'hosting-php': {
      ...
      subtitle: 'Hosting Only (Tanpa Domain)',
      color: 'cyan',
      segments: [
        {
          duration: '1 Bulan',
            ...
            {pricingData[activeTab].segments.map((segment, segmentIndex) => {
              ...
              return (
                ...
                >
                  ...
                  {segment.duration && (
                    <div className={`${colors.bg} text-white px-6 py-4 rounded-t-lg`}>
                  ...
                  {/* Pricing Table */}
                  <div className={`bg-white rounded-lg ${segment.duration ? 'rounded-t-none' : ''} border-2 ${colors.border} overflow-hidden shadow-lg`}>
                    {/* Table Header */}
                    <div className="grid grid-cols-3 gap-4 p-4 sm:p-6 bg-gray-50 border-b-2 border-gray-200">
                      <div className="font-bold text-gray-900 text-sm sm:text-base">Nama Produk</div>
                      <div className="font-bold text-gray-900 text-center text-sm sm:text-base">Spesifikasi</div>
                      <div className="font-bold text-gray-900 text-center text-sm sm:text-base">Harga (IDR)</div>
                    </div>
                    ...
                    <div>
                      {segment.packages.map((pkg, index) => (
                        <motion.div
                          ...
                          className={`grid grid-cols-3 gap-4 p-4 sm:p-6 border-b border-gray-100 ${colors.hover} transition-colors ${
                            index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'
                          }`}
                        >
                          <div className="flex items-center">
                            <span className={`px-3 sm:px-4 py-2 ${colors.bg} text-white font-bold rounded-lg text-xs sm:text-sm`}>
                              {pkg.name}
                            </span>
                          </div>
                          <div className="flex items-center justify-center text-gray-700 text-xs sm:text-sm">
                            {pkg.spec}
                          </div>
                          <div className="flex items-center justify-center font-bold text-gray-900 text-sm sm:text-base">
                            {pkg.price}
                          </div>
                        </motion.div>
                      ))}
